...
dens_never <- density(never_smoked$wt)
plot(dens_never, main = "Density Plot of Birth Weights (Never Smoked)", 
     xlab = "Birth Weight (ounces)", ylab = "Density")
lines(dens_never, col = "blue")

dens_now <- density(smokes_now$wt)
plot(dens_now, main = "Density Plot of Birth Weights (Smokes Now)", 
     xlab = "Birth Weight (ounces)", ylab = "Density")
lines(dens_now, col = "red")

par(mfrow = c(1, 1))  # Reset layout to a single plot
